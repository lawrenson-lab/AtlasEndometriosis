...
table(aux.seurat@meta.data$selected.cells)
table(aux.seurat@meta.data$subcluster.name)
...
table(sc.selected@meta.data$active.cluster)
table(sc.selected@meta.data$subcluster.name)



# class = "ARID1A_Status"
# markers = c("ARID1A")
# DimPlot(aux.seurat.endo, reduction = "umap")
# data.table.filt = data.table[which(data.table$ARID1A != "NP"), ]
# 
# sc.sel.filt = subset(aux.seurat.endo, subset = SampleName %in% data.table.filt$SampleName)
# 
# sc.sel.filt <- AddMetaData(sc.sel.filt, metadata=NA, col.name="Status")
# sc.sel.filt@meta.data$Status[which(sc.sel.filt@meta.data$SampleName %in% data.table.filt$SampleName[data.table.filt$ARID1A == "Positive"])] <- "Positive"
# sc.sel.filt@meta.data$Status[which(sc.sel.filt@meta.data$SampleName %in% data.table.filt$SampleName[data.table.filt$ARID1A %in% c("Heterogenous")])] <- "Heterogenous"
# 
# 
# head(data.table)
...
table(Idents(sel.endo))
#DimPlot(sel.endo, group.by = "SampleName", reduction = "umap")
...
table(sel.endo@meta.data$ARID1A_status)
...
p1 <- ggplot(counts.prop.perc, aes(x = reorder(Var1, percent), y = percent, fill = Var2, label = signif(round(percent, digits = 3), digits = 3))) + 
...
table(.sc.epi@meta.data$selected.cells)
